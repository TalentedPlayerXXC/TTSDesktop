...
- **角色数据本地化** — 349 个角色 + 22 个标签从 MongoDB 导出为本地 JSON，启动秒加载，移动宽带用户不再白屏
- **收藏持久化** — 收藏的角色关闭应用不丢失，自动清洗失效 ID，不留脏数据
- **自定义配音员管理** — 配音页 hover 显示 × 删除按钮，设置页支持一键全部删除

### 🎨 UI 整活

- **骨架屏 0.5s 延迟** — 虽然数据本地读只需毫秒，但故意延迟半秒，制造「正在请求」的假象 🤡
- **自定义配音员 × 按钮** — 移到卡片左上角，跟右上角收藏不打架
