...
- **架构文档页上线** — `huisheng-client.html` 前端架构 + `huisheng-serve.html` 后端架构，全展开可折叠，SVG 架构图、代码片段、数据流、端点表格一条龙。再也不怕别人问「你这项目怎么搭的」🤓
- **右上角导航三连** — 发布页顶部加了「📐 前端架构 / ⚙️ 后端架构」快捷跳转，两个架构页也互相链 + 链回发布页，三个页面随便跳转不带迷路 🧭
- **星标（⭐）移到导航最右边** — 跟架构链接排排坐，保持一致
- **架构页也有星标了** — 两个架构页右上角也加了 GitHub Star 数，实时从 API 拉，跟发布页同步 ✨
- **导航全用相对路径** — 双击本地文件也能正常跳转，不挑环境 👌
...
- **electron-builder 配置加固** — 两个 yml 都加了 `identity: null`，签名行为更可控

### 🔒 安全修复

- **反馈日志路径脱敏** — 提交 Issue 前自动将日志中的绝对路径裁剪为 `/最后/两段`，用户名和目录结构不再暴露。修复 `Application Support` 等含空格路径的匹配问题 🛡️
- **删除联系方式收集** — 反馈弹窗去掉联系方式输入框，Issue 正文不再附带 `📧 联系方式`。GitHub 是公开平台，不收集任何个人可识别信息 🚫

### 🎨 分发站优化

- **右上角 Star ⭐** — GitHub Pages 下载页右上角加了 star 徽章，自动显示 star 数，悬停高亮，点它直达仓库
- **项目简介** — 标题下加了俏皮的功能简介，一看就知道是干啥的
- **内嵌更新日志** — 页面底部折叠展示 `UPDATE.md`（从 dev 分支实时拉取），懒得去 GitHub 也能直接看
...
- **情感解析正则增强** — 新增 `【情绪】` 纯标签匹配规则，`【默认2】` / `【默认3】` 这类带编号的情感终于能正确识别并展示在情感列表里了 👀
- **包名全改英文** — artifactName 从 `绘声-1.2.0-online-arm64.dmg` 改为 `huisheng-1.2.0-online-arm64.dmg`，Github 不再抽风

### 🧹 数据清理

- **`【_unk_】` → `【其他】`** — 21 个无法归类的音频文件批量改名，不再显示丑陋的 `_unk_`，统一展示为「其他」
...
- **📜 启动免责声明** — 每次启动弹出使用条款，不同意直接秒退进程，干干净净
